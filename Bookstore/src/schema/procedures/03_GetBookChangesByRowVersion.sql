CREATE OR ALTER PROCEDURE [dbo].[GetBookChangesByRowVersion]
(
   @startRow BIGINT
   ,@endRow  BIGINT
)
...
BEGIN
  SELECT b.[book_id]
    ...
    LEFT JOIN [dbo].[book_author] ba ON b.book_id = ba.book_id
   ...
   OR (a.[rowversion] > CONVERT(ROWVERSION,@startRow) AND a.[rowversion] <= CONVERT(ROWVERSION,@endRow))
    GROUP BY b.[book_id], b.[title], b.[isbn13], bl.[language_code], bl.[language_name], b.[num_pages], b.[publication_date], p.[publisher_name]
END
